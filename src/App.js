...
function App() {
	...
	useEffect(() => {
		if (Count< 2) {
			...
			fetch(`https://pokeapi.co/api/v2/pokemon/${Data}`, {
				...
				.then((res) => res.json())
				.then((data) =>
					setRes((prev) => [...prev, `muneeb@me# `, `Name: ${data.name}` , `Height: ${data.height}`, `Weight: ${data.weight}`, `Types: ${data.types[0].type.name}`])
				)
				.catch((err) => {
					setRes((prev) => [ ...prev, "muneeb@me# Enter a valid poke`mon"]) 
					console.log(err)}
				);
		}
		return () => {};
	...
	const verify = (key) => {
		...
		switch (key.split(" ")[0]) {
			...
			case `whois`:
				return [`muneeb@me# the author of this project is me, Muneeb Mohd. To know more about me visit my portfolio!`];

			case `about`:
				return [`muneeb@me# I would like to start this by thanking omDb who's API service made this app possible. A huge thanks to them! Do visit them on url`];
			...
			case `portfolio`:
				window.open(
					`http://www.mohdmuneeb.live`,
					`_blank`
				);
				break;
			case `history`:
				return [`muneeb@me# History for this current session :- ${History}`];
			default:
				return [`muneeb@me# Wrong Command has been entered, please use <help> command to guide you with this website`];
		}
	};
